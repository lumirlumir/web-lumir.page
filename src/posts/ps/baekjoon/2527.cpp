...
int main(int argc, char* argv[]) {
  for (int i = 0; i < 4; i++) {
    /* Input */
    Square s1, s2;
    scanf("%d %d %d %d %d %d %d %d", &s1.ld.x, &s1.ld.y, &s1.ru.x, &s1.ru.y, &s2.ld.x,
          &s2.ld.y, &s2.ru.x, &s2.ru.y);
    ...
    if ((s1.ru.x > s2.ld.x && s1.ru.y > s2.ld.y) &&
      ...
      printf("a\n");
             ...
             (s1.ru.y == s2.ld.y && s1.ru.x > s2.ld.x && s1.ld.x < s2.ru.x) ||  // 위쪽면
             (s1.ld.x == s2.ru.x && s1.ld.y < s2.ru.y && s1.ru.y > s2.ld.y) ||  // 왼쪽면
             (s1.ld.y == s2.ru.y && s1.ld.x < s2.ru.x && s1.ru.x > s2.ld.x))  // 아래쪽면
      printf("b\n");
